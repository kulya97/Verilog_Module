...
module uart_bit_tx_module #(
  ...
  reg [3:0] state;
  reg [3:0] next_state;
  localparam S_IDLE = 4'b0001;  //空闲
  localparam S_START = 4'b0011;  //起始位
  localparam S_SEND_BYTE = 4'b0111;  //数据位
  localparam S_STOP = 4'b1111;  //停止位
  /***********************************************************************/
  ...
  always @(*) begin
    case (state)
      S_IDLE:
      if (tx_data_valid && tx_data_ready) next_state <= S_START;
